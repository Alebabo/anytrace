...
Upload the full `landingpage-anytrace` folder to your hosting and use its contents as the web root.

## Screenshot Map

Replace these files in `landingpage-anytrace/assets/` with your real screenshots:
...
- `screenshot-graph.svg`
  ...
  Best screenshot: the real AnyTrace graph page with a centered selected node and visible connections.
...
- Graph screenshot:
  Show the actual AnyTrace graph canvas with the side panel open if possible.
...
- The header logo uses `assets/anytrace-logo.png`, copied from the app assets.
- If you have a final domain, add `canonical` and `og:url` to `index.html`.
- Update `hello@anytrace.app` if you want a different CTA destination.
